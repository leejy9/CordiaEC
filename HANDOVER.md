...
CMS를 새로 구축하실 2차 개발사가 참고하실 부분입니다.

### 8.1 이미지 저장 방식
- 현재 뉴스/게시물 이미지는 **base64 데이터 URL로 DB에 직접 저장**됩니다.
- 글이 많아질수록 DB 용량이 빠르게 증가합니다.
- **개선 권장**: 이미지 업로드를 S3 / Supabase Storage / Cloudflare R2 등으로 분리.

### 8.2 요청 본문 크기 제한
- 위 base64 저장 구조 때문에 Express의 body limit이 **10mb**로 설정되어 있습니다 (`server/index.ts`).
- 이미지 스토리지를 분리하면 이 값을 줄여도 됩니다.

### 8.3 인증 시스템 부재
- 일반 사용자 인증 기능이 없습니다.
- 추후 회원 가입·로그인 기능이 필요하면 새로 설계 필요.

### 8.4 Replit 의존성
- `package.json`에 `@replit/vite-plugin-cartographer`, `@replit/vite-plugin-runtime-error-modal`이 포함되어 있습니다.
- 외부 환경에서는 불필요하므로 `cleanup-for-deployment.sh` 스크립트로 제거 가능합니다.
- 자세한 외부 배포 절차는 **`DEPLOYMENT.md`** 파일 참조.

### 8.5 사용하지 않는 코드
- 이전에 존재하던 `initiatives` DB 테이블 및 관련 API는 모두 정리되었습니다.
- 추가 정리가 필요한 부분은 발견되지 않았으나, 운영하시며 필요 시 리팩터링 권장.
...
### 인계받는 개발사가 해야 할 일
- [ ] 코드 전달 받기 (Git 레포 또는 zip)
- [ ] 자체 PostgreSQL DB 준비 후 `DATABASE_URL` 설정
- [ ] `npm install` → `npm run db:push` → `npm run dev`로 로컬 구동 확인
- [ ] **관리자 비밀번호 변경** (`client/src/pages/Admin.tsx`)
- [ ] 자체 호스팅 환경(Vercel, Railway, AWS 등)에 배포
- [ ] 도메인 연결 및 HTTPS 설정
- [ ] (권장) 관리자 인증을 백엔드 기반으로 개선
- [ ] (권장) 이미지 저장을 외부 스토리지로 분리

### 인계하는 측(고객)이 정리할 일
- [ ] 기존 운영 중인 사이트가 있다면 중단 시점 결정
- [ ] 도메인 DNS를 새 서버로 전환
- [ ] 기존 DB(Supabase 등) 비용 발생 여부 확인 후 정리
- [ ] 디자인 시안·로고 원본·브랜드 가이드가 있다면 함께 전달

---

## 11. 문의

이 문서나 코드 구조에 대해 추가 설명이 필요하시면 1차 개발사로 연락 주시기 바랍니다.
